...
(function() {
  var BaseEmitter, is_primitive, path_lib, pkg;
  ...
  exports.BaseEmitter = BaseEmitter = (function() {
    ...
    BaseEmitter.prototype.get_field_type = function(type) {
      if (typeof type === 'string') {
        ...
        if (Array.isArray(type)) {
          ...
          return this.get_field_type(type.items);
        } else if (type.type === 'map') {
          return this.get_field_type(type.values);
        }
      }
    };

    BaseEmitter.prototype.create_dep_graph = function(types) {
      var case_, case_type, field, field_type, graph, type, _i, _j, _k, _l, _len, _len1, _len2, _len3, _ref, _ref1;
      graph = {};
      ...
      for (_j = 0, _len1 = types.length; _j < _len1; _j++) {
        ...
        switch (type.type) {
          case "record":
            if (type.typedef && !is_primitive(type.typedef)) {
              ...
              for (_k = 0, _len2 = _ref.length; _k < _len2; _k++) {
                ...
                if (!is_primitive(field_type) && !graph[field_type].children.includes(type.name)) {
          ...
          case "variant":
            ...
            for (_l = 0, _len3 = _ref1.length; _l < _len3; _l++) {
              ...
              case_type = typeof case_.body === 'object' ? case_.body.items : case_.body;
              if (!is_primitive(case_type) && !graph[case_type].children.includes(type.name)) {
    ...
    BaseEmitter.prototype.sort_types = function(types) {
      var child, graph, prev_length, res_types, sources, type, _i, _j, _len, _len1, _ref;
      graph = this.create_dep_graph(types);
      console.log('graph:', graph);
      res_types = [];
      prev_length = Object.keys(graph).length;
      while (Object.keys(graph).length > 0) {
        sources = Object.values(graph).filter((function(_this) {
          return function(node) {
            return node.in_count === 0;
          };
        })(this));
        ...
        if (Object.keys(graph).length === prev_length) {
          res_types.concat(Object.keys(graph).map(function(type) {
            return type.type;
          }));
          break;
        }
    ...
    BaseEmitter.prototype.run = function(_arg) {
      ...
      infiles.sort();
      json.types = this.sort_types(json.types);
      this.create_dep_graph(json.types);
