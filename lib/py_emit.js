...
(function() {
  var BaseEmitter, PythonEmitter, camelCase, isEqual, path_lib, pkg, uniqWith, _ref,
  ...
  _ref = require('lodash'), uniqWith = _ref.uniqWith, isEqual = _ref.isEqual, camelCase = _ref.camelCase;
  ...
  exports.PythonEmitter = PythonEmitter = (function(_super) {
    ...
    function PythonEmitter() {
      ...
      this._keywords = new Set(['False', 'None', 'True', 'and', 'as', 'assert', 'break', 'class', 'continue', 'def', 'del', 'elif', 'else', 'except', 'finally', 'for', 'from', 'global', 'if', 'import', 'in', 'is', 'lambda', 'nonlocal', 'not', 'or', 'pass', 'raise', 'return', 'try', 'while', 'with', 'yield']);
    }
    ...
    PythonEmitter.prototype.emit_imports = function(_arg, outfile) {
      var import_as, imports, path, _i, _len, _ref1;
      imports = _arg.imports;
      this.output("from dataclasses import dataclass");
      this.output("from enum import Enum");
      this.output("from typing import Dict, List, Optional, Union");
      this.output("");
      this.output("from mashumaro import DataClassJSONMixin");
      this.output("");
    ...
    PythonEmitter.prototype.emit_field = function(_arg) {
      ...
      return this.output("" + fieldName + ": " + type);
    };
          ...
          bodyType = (function() {
            switch (false) {
              ...
              case typeof type_case.body !== 'string':
                return this.emit_primitive_type(type_case.body);
              case type_case.body.type !== 'array':
                return "List[" + (this.emit_primitive_type(type_case.body.items)) + "]";
              default:
                throw new Error("Unrecognized type");
            }
          }).call(_this);
          _this.output("@dataclass");
          _this.output("class " + type.name + type_case.label.name + ":");
          _this.tab();
          _this.output("" + type["switch"].name + ": " + type["switch"].type + "." + type_case.label.name);
          _this.output("" + type_case.label.name + ": Optional[" + bodyType + "]");
          _this.untab();
          return "" + type.name + type_case.label.name;
        };
      })(this)).filter(Boolean);
