...
(function() {
  ...
  exports.GoEmitter = GoEmitter = (function(_super) {
    ...
    function GoEmitter(gomod_path, gomod_dir) {
      this.gomod_path = gomod_path;
      this.gomod_dir = gomod_dir;
    ...
    GoEmitter.prototype.codec = function(_arg) {
      var jsonkey, mpackkey, name, null_serializable, omitempty, optional;
      name = _arg.name, optional = _arg.optional, jsonkey = _arg.jsonkey, mpackkey = _arg.mpackkey, null_serializable = _arg.null_serializable;
      omitempty = optional && !null_serializable ? ",omitempty" : "";
      if (jsonkey == null) {
        jsonkey = name;
      }
      if (mpackkey == null) {
    ...
    GoEmitter.prototype.emit_field = function(_arg) {
      var cols, exported, go_field_suffix, jsonkey, mpackkey, name, null_serializable, optional, optionalkey, pointed, type, _ref;
      name = _arg.name, type = _arg.type, go_field_suffix = _arg.go_field_suffix, exported = _arg.exported, optionalkey = _arg.optionalkey, pointed = _arg.pointed, jsonkey = _arg.jsonkey, mpackkey = _arg.mpackkey, null_serializable = _arg.null_serializable;
    ...
    GoEmitter.prototype.deep_copy_pointer = function(_arg) {
      var can_simplify, inner, inner_val, t, type, val;
      t = _arg.t, val = _arg.val;
      type = this.emit_field_type(t).type;
      this.deep_copy_preamble({
        type: type
      ...
      inner = t[1];
      can_simplify = typeof inner === 'string' && inner !== 'bytes' && !this.is_primitive_type_lax(inner);
      inner_val = can_simplify ? "x" : "(*x)";
      this.deep_copy({
        t: inner,
        val: inner_val,
        exported: true
    ...
    GoEmitter.prototype.emit_variant_case_getter = function(_arg) {
      var c, cases, def, go_field_suffix, go_label, obj, optional, ret_type, switch_field, tag_val, type, v, _ref;
      ...
      if (def) {
        ...
        switch_field = "o." + (this.variant_field(obj["switch"].name));
        if (tag_val === "true" || tag_val === true) {
          this.output("if !" + switch_field + " {");
        } else if (tag_val === "false" || tag_val === false) {
          this.output("if " + switch_field + " {");
        } else {
          this.output("if " + switch_field + " != " + tag_val + " {");
        }
      }
      this.tab();
    ...
    GoEmitter.prototype.emit_enum = function(t, nostring) {
      ...
      this.output("}");
      if (!nostring) {
        this.output("func (o " + name + ") String() string {");
        this.tab();
        this.output("if v, ok := " + name + "RevMap[o]; ok {");
        this.tab();
        this.output("return v");
        this.untab();
        this.output("}");
        this.output("return fmt.Sprintf(\"%v\", int(o))");
        this.untab();
        return this.output("}");
      }
    };
    ...
    GoEmitter.prototype.emit_imports = function(_arg, outfile, _arg1) {
      var has_messages, import_as, imports, line, messages, path, relative_dir, relative_file, types, types_only, _i, _len, _ref;
      ...
      relative_file = path_lib.resolve(outfile).replace(this.gomod_dir, "");
      relative_dir = this.gomod_path + path_lib.dirname(relative_file);
    ...
    GoEmitter.prototype.emit_server_hook_make_handler = function(_arg) {
      ...
      this.output("Handler: func(ctx context.Context, args any) (ret any, err error) {");
      this.tab();
      if (arg.nargs > 0) {
    ...
    GoEmitter.prototype.emit_preface = function(infiles, json, _arg) {
      ...
      this.output("// Code generated to Go " + (types_only ? 'types' : 'types and interfaces') + " using " + pkg.name + " v" + pkg.version + " (" + pkg.homepage + "). DO NOT EDIT.");
      if (infiles.length === 1) {
